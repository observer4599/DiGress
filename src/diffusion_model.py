...
class LiftedDenoisingDiffusion(pl.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        self.print(
            f"Epoch {self.current_epoch}: X_mse: {to_log['train_epoch/epoch_X_mse']:.3f}"
    ...
    def compute_val_loss(
        self, pred, noisy_data, X, E, y, node_mask, test=False
    ):
        ...
        if test:
            diffusion_error = (
                self.test_X_mse(
                    weighted_predX_diffusion, weighted_epsX_diffusion
                )
                + self.test_E_mse(
                    weighted_predE_diffusion, weighted_epsE_diffusion
                )
                + self.test_y_mse(
                    weighted_predy_diffusion, weighted_epsy_diffusion
                )
            )
        else:
            diffusion_error = (
    ...
    def sample_discrete_graph_given_z0(self, X_0, E_0, y_0, node_mask):
        ...
        pred_E = (
            1.0 / alpha_0.unsqueeze(1) * (E_0 - sigma_0.unsqueeze(1) * eps0.E)
        )
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        sigma2_t_given_s, sigma_t_given_s, alpha_t_given_s = (
            diffusion_utils.sigma_and_alpha_t_given_s(
        ...
        mu_E = (
            ...
            - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)).unsqueeze(1)
            * eps.E
        )
        mu_y = (
            y_t / alpha_t_given_s.squeeze(1)
            - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)).squeeze(1)
            * eps.y
        )
        ...
        z_s = diffusion_utils.sample_normal(
            mu_X, mu_E, mu_y, sigma, node_mask
        ).type_as(mu_X)
